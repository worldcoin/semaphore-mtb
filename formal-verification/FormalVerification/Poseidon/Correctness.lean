...
lemma sbox_uncps (A : F) (k : F -> Prop): VerifyProof.sbox A k = k (A ^ 5) := by
  unfold VerifyProof.sbox
  simp [Gates.mul]
  apply iff_to_eq
  repeat (rw [pow_succ])
...
lemma mds_3_uncps (S : Vector F 3) (k : Vector F 3 -> Prop):
  VerifyProof.mds_3 S k = k (mds_matmul Constants.x5_254_3 S) := by
  simp [VerifyProof.mds_3, Gates.add, Gates.mul]
  apply iff_to_eq
...
lemma full_round_3_uncps (S C: Vector F 3) (k : Vector F 3 -> Prop):
  VerifyProof.fullRound_3_3 S C k = k (full_round Constants.x5_254_3 S C) := by
  unfold VerifyProof.fullRound_3_3
  unfold Gates.add
...
lemma half_round_3_uncps (S C: Vector F 3) (k : Vector F 3 -> Prop):
  VerifyProof.halfRound_3_3 S C k = k (partial_round Constants.x5_254_3 S C) := by
  unfold VerifyProof.halfRound_3_3
  unfold Gates.add
...
lemma partial_rounds_3_uncps
  ...
  {start count : Nat}
  {k : Vector F 3 -> Prop}:
  half_rounds_cps Constants.x5_254_3 VerifyProof.halfRound_3_3 S start count k = k (partial_rounds Constants.x5_254_3 S start count) := by
...
lemma full_rounds_3_uncps
  ...
  {start count : Nat}
  {k : Vector F 3 -> Prop}:
  full_rounds_cps Constants.x5_254_3 VerifyProof.fullRound_3_3 S start count k = k (full_rounds Constants.x5_254_3 S start count) := by
...
def looped_poseidon_3 (inp : Vector F 3) (k: Vector F 3 -> Prop): Prop :=
    full_rounds_cps Constants.x5_254_3 VerifyProof.fullRound_3_3 inp 0 4 fun state =>
    half_rounds_cps Constants.x5_254_3 VerifyProof.halfRound_3_3 state 12 57  fun state' =>
    full_rounds_cps Constants.x5_254_3 VerifyProof.fullRound_3_3 state' 183 4 k
...
theorem looped_poseidon_3_go (inp : Vector F 3) (k : Vector F 3 -> Prop):
    VerifyProof.poseidon_3 inp k = looped_poseidon_3 inp k := by
    unfold looped_poseidon_3
    unfold VerifyProof.poseidon_3
...
theorem poseidon_3_correct (inp : Vector F 3) (k : Vector F 3 -> Prop):
  VerifyProof.poseidon_3 inp k = k (Poseidon.perm Constants.x5_254_3 inp) := by
  simp [
