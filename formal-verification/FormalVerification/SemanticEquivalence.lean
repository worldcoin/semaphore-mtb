...
abbrev gVerifyProof := SemaphoreMTB.VerifyProof_4_3
abbrev gDeletionRound := SemaphoreMTB.DeletionRound_3_3
abbrev gDeletionProof := SemaphoreMTB.DeletionProof_2_2_3_2_2_3
...
lemma select_is_match {b i1 i2 out : F} : (Gates.select b i1 i2 out) ↔ match zmod_to_bit b with
  | Bit.zero => i2 = out
  | Bit.one => i1 = out := by sorry

lemma sub_zero_is_eq {a b skip : F} : (
    Gates.is_zero (Gates.sub a b) gate_4 ∧
    Gates.or gate_4 skip gate_5 ∧
    Gates.eq gate_5 (1:F)) ↔ (a = b ∨ skip = 1) := by sorry

  -- ↔ a = b ∨ out = 1:= by
  -- simp [Gates.sub, Gates.is_zero]
  -- sorry

-- lemma or_is_match {a b out : F} : (Gates.or a b out ∧ Gates.eq out (1:F)) ↔ match zmod_to_bit a with
--   | Bit.zero => b = 1
--   | Bit.one => a = 1 := by sorry
...
lemma DeletionRound_uncps {Root: F} {Index: F} {Item: F} {Proof: Vector F D} {k: F -> Prop} :
  ...
  (MerkleTree.recover_tail poseidon₂ ((Dir.create_dir_vec out).take D) Proof Item - Root = 0 ∨ out.get D = 1) ∧
  k (match zmod_to_bit (out.get D) with
    | Bit.zero => MerkleTree.recover_tail poseidon₂ ((Dir.create_dir_vec out).take D) Proof 0 -- Update the root
    | Bit.one => Root  -- Skip flag set, don't update the root
  ) := by
  unfold gDeletionRound
  -- Is there a way to automatically unfold from gDeletionRound?
  unfold SemaphoreMTB.DeletionRound_3_3
  simp [Gates.to_binary, Gates.sub, Gates.is_zero, Gates.or, Gates.eq]
...
-- -/
set_option trace.Debug.Meta.Tactic.simp false
...
def DeletionLoop {n} (DeletionIndices: Vector F n) (PreRoot: F) (IdComms: Vector F n) (MerkleProofs: Vector (Vector F D) n) (k : F -> Prop) : Prop :=
  ...
  | Nat.succ _ =>
    ...
    (MerkleTree.recover_tail poseidon₂ ((Dir.create_dir_vec out).take D) MerkleProofs.head IdComms.head - PreRoot = 0 ∨ out.get D = 1) ∧
    DeletionLoop DeletionIndices.tail ((match zmod_to_bit (out.get D) with
                                        | Bit.zero => MerkleTree.recover_tail poseidon₂ ((Dir.create_dir_vec out).take D) MerkleProofs.head 0 -- Update the root
                                        | Bit.one => PreRoot  -- Skip flag set, don't update the root
    )) IdComms.tail MerkleProofs.tail k
